...
void InjectDll(HANDLE hThread, DWORD processId, LPCWSTR dllName) {
	HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, processId);
	LPVOID address = NULL;
	address = VirtualAllocEx(hProcess, NULL, lstrlenW(dllName)*sizeof(TCHAR), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	SIZE_T bytesWriten;
	WriteProcessMemory(hProcess, address, dllName, lstrlenW(dllName) * sizeof(TCHAR), &bytesWriten);
	QueueUserAPC((PAPCFUNC)LoadLibraryW, hThread, (ULONG_PTR)address);
	VirtualFreeEx(hProcess, address, bytesWriten, MEM_DECOMMIT | MEM_RELEASE);
	CloseHandle(hThread);
	CloseHandle(hProcess);
	QueueUserAPC((PAPCFUNC)LoadLibraryW, hThread, (ULONG_PTR)dllName);
}
...
int main() {
	...
	InjectDll(hAlertableThread, processId, DLL_PATH);
	return 1;
}